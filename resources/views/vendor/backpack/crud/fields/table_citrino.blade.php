...
@include('crud::fields.inc.wrapper_start')
    ...
    <input class="array-json"
            type="hidden"
            data-init-function="bpFieldInitTableElement2"
            name="{{ $field['name'] }}"
            value="{{ $items }}"
            data-max="{{$max}}"
            data-min="{{$min}}"
        ...
        <table class="table table-sm table-striped m-b-0">
            ...
            <thead>
                <tr class="row">
                    ...
                    <th style="font-weight: 600!important;" class="{{ $column['class'] }}">
                        {{ $column['label'] }}
                    </th>
                    @endforeach
                    <th class="text-center col-sm-1"> {{-- <i class="la la-sort"></i> --}} </th>
                    <th class="text-center col-sm-1"> {{-- <i class="la la-trash"></i> --}} </th>
                </tr>
            </thead>

            <tbody class="table-striped items sortableOptions">

                <tr class="array-row row clonable" style="display: none;">
                    @foreach( $field['columns'] as $name => $column)
                        @if($column['type']== 'textarea')
                            <td class="{{ $column['class'] }}">
                                <textarea data-cell-name="item.{{ $name }}" rows="4" class="form-control "></textarea>
                            </td>
                        @else
                            <td class="{{ $column['class'] }}">
                    ...
                    @endforeach
                    <td class="col-sm-1">
                        <span class="btn btn-light sort-handle pull-right"><span class="sr-only">sort item</span><i class="la la-sort" role="presentation" aria-hidden="true"></i></span>
                    </td>
                    <td class="col-sm-1">
                        <button class="btn  btn-light removeItem" type="button"><span class="sr-only">delete item</span><i class="la la-trash" role="presentation" aria-hidden="true"></i></button>
                    </td>
                </tr>
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            function bpFieldInitTableElement2(element) {
                ...
                if($rows != '[]') {
                    $.each($rows, function(key) {
                        ...
                        $.each(this, function(column , value) {
                            $tableWrapper.find('tbody tr:last').find('[data-cell-name="item.' + column + '"]').val(value);
                        });
